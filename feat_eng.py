...
class DropModeAVG(BaseEstimator, TransformerMixin):
    """
    This transformer identifies and drops features in a DataFrame with names ending in specified suffixes, such as "_AVG" or "_MODE".
    Certain features, specified in the 'exceptions' parameter, can be excluded from the dropping process.
    """
    ...
    def transform(self, X):
        features_to_drop = [
            ...
            if any(col.endswith(suffix) for suffix in self.suffixes)
            and col not in self.exceptions
        ]
...
class PreviousApplicationTransformer(BaseEstimator, TransformerMixin):
    ...
    3. Aggregates RATE_DOWN_PAYMENT, calculating median, max, and min values for each SK_ID_CURR.
    ...
    def aggregate_rate_down_payment(self, X):
        rate_down_payment_agg = (
            X.groupby("SK_ID_CURR")["RATE_DOWN_PAYMENT"]
            .agg(["median", "mean", "max", "min"])
            .reset_index()
        )
        rate_down_payment_agg = rate_down_payment_agg.rename(
            columns=lambda col: f"PREV_rate_down_payment_{col}"
            if col != "SK_ID_CURR"
            else col
        )
        return rate_down_payment_agg
...
class BureauTransformer(BaseEstimator, TransformerMixin):
    ...
    Transforms the bureau dataframe by calculating:
     ...
     2- creating a credit active pivot table,
     3- financial ratios (median, max, and min values for each SK_ID_CURR).
    ...
    def transform(self, X):
        ...
        X_transformed = bureau_loan_counts.merge(
            pivot_table, on="SK_ID_CURR", how="left"
        )
        X_transformed = X_transformed.merge(
